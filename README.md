...
> **PLEASE NOTE**
> `tcp`, `tcp6`, `udp`, `udp6`: Collecting messages from remote hosts using the BSD syslog protocol are **OBSOLETE**
> Use the `network()` source and the `network()` destination instead.
...
At present, 2 targets can be used:

- `file`, the writing of a local logfile ([see also](https://www.syslog-ng.com/technical-documents/doc/syslog-ng-open-source-edition/3.18/administration-guide/33#TOPIC-1044005))
- `syslog`, sending messages to a remote logserver using the IETF-syslog protocol ([see also](https://www.syslog-ng.com/technical-documents/doc/syslog-ng-open-source-edition/3.18/administration-guide/46#TOPIC-1044060))
- `syslog-ng`, (*planned*) forwarding messages and tags to another syslog-ng node ([see also](https://www.syslog-ng.com/technical-documents/doc/syslog-ng-open-source-edition/3.18/administration-guide/47#TOPIC-1044062))

Additional log sources to be merged with the [default](./vars/main.yml) (`syslog_defaults_destinations`) ones.

| parameter           | required | default           | description                                      |
| :----               | :----    | :----             | :-----                                           |
| `file_name`         | `false`  | `${key}.log`      | log file relative to `/var/log`. (The basic directory is created automatically.) |
| `template`          | `false`  | `-`               | use the created template for this entry |
| `use_template`      | `false`  | `true`            | enable or disable the usage of an template |
| `syslog`            | `false`  | `-`               | |
| `transport`         | `false`  | `udp`             | specifies the protocol used to send messages to the destination server. |
| `port`              | `false`  | `514`             | specifies the port used to send messages to the destination server.<br>The default ports for the different transport protocols are as follows: UDP: 514, TCP: 601, TLS: 6514. |
